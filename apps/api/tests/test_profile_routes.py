...
def test_profile_returns_defaults_for_authorized_user(
    ...
    login(client, "soviedo@ucenfotec.ac.cr", "1234ABC")
...
def test_profile_can_be_updated_and_persisted(
    ...
    refreshed_user = (
        db_session.query(User).filter(User.email == "soviedo@ucenfotec.ac.cr").one()
    )
...
def test_password_change_updates_credentials(
    client: TestClient, db_session: Session
) -> None:
    create_user(
        db_session,
...
def test_password_change_enforces_rules(
    client: TestClient, db_session: Session
) -> None:
    create_user(
        db_session,
