...
def test_manual_upload_requires_admin_role(client: TestClient, db_session: Session) -> None:
    create_user(
        db_session,
    ...
    assert response.json()["detail"] == "Se requiere una sesion activa con rol de administrador."
...
def test_manual_upload_persists_metadata_and_stores_pdf(
    ...
    try:
        ...
        response = client.post(
            ...
            files={"file": ("rc7-programmer-manual.pdf", b"%PDF-1.4 manual content", "application/pdf")},
        )
...
def test_manual_list_and_detail_return_registered_manuals(
    ...
    assert [item["title"] for item in payload["items"]] == ["Manual Nuevo", "Manual Antiguo"]
...
def test_manual_upload_rejects_non_pdf_files(client: TestClient, db_session: Session) -> None:
    create_user(
        db_session,
        email="admin@ucenfotec.ac.cr",
        password="1234ABC",
