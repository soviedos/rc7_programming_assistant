...
class ManualStorageService:
    """MinIO client for the manuals bucket.

    Takes the settings explicitly so api and worker can each pass their own
    ``Settings`` (both inherit the minio_* fields from ``SharedSettings``).
    """
    ...
    def __init__(
        ...
        self._config = config
        self.bucket_name = bucket_name or config.minio_bucket_manuals
        self.client = client or self._build_client()

    def _build_client(self) -> Minio:
        parsed_endpoint = urlparse(self._config.minio_endpoint)
