...
| Autofix `regenerate` aplicado | `regenerate` no implementado; tratado como `keep` | [jobs/ingestion.py](../../apps/worker/src/jobs/ingestion.py) | Aclarado |
...
| API tiene setting `gemini_model` | **Corregido (código):** los modelos se centralizaron en `config.py` (`gemini_model`, `gemini_embedding_model`), overridables por env. No son settings de DB en caliente. | [core/config.py](../../apps/api/src/core/config.py), [chat/service.py:20-22](../../apps/api/src/services/chat/service.py#L20) | Corregido (CODE_AUDIT D1) |
| Settings en caliente: `gemini_temperature`, `gemini_max_tokens`, `rag_top_k_chunks`, `rag_context_budget_chars`, `system_prompt_pac`, `history_max_entries` | Todas se leen en runtime vía `get_setting_value` | [chat/service.py](../../apps/api/src/services/chat/service.py), [chat.py:62](../../apps/api/src/api/v1/routes/chat.py#L62) | Verificado |
